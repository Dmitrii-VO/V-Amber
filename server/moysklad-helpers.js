...
//
// Даёт две вещи: точный поиск заказа при неизвестном исходе (filter=syncId=...)
// вместо угадывания по описанию, и — если МойСклад трактует syncId как ключ
// upsert — невозможность создать дубль на его стороне вообще. Вторая часть на
// момент написания не подтверждена: с машины разработчика api.moysklad.ru
// недоступен (см. scripts/probe-moysklad-syncid.mjs). Код на неё не
// закладывается, защита держится журналом и поиском по syncId.

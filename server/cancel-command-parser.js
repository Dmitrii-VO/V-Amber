...
export function parseCancelCommand(text) {
  ...
  // Код ищем ТОЛЬКО после триггера («...лота #033322»). Раньше цифровой
  // поиск шёл по всему тексту, и упомянутая до команды цена/количество
  // перехватывала код: «за 2500 галина прокофьева отмена брони ноль три
  // три три два два» давало code=2500 — подсветилась бы чужая бронь.
  // Отмена — это реальные деньги: лучше отказаться, чем угадать не то.
  // Сначала цифровая запись, затем словесная («ноль один ноль…»), которую
  // оператор произносит чаще всего, когда диктует код вслух.
  const tail = normalized.slice(triggerMatch.index + triggerMatch[0].length);
  const codeMatch = CODE_RE.exec(tail);
  const code = codeMatch ? codeMatch[1] : extractDigitWordsCode(tail);
...
// вводные («так», «давай», «значит» и т.п.) и предлоги с начала, оставляя
// хвост, который и есть «Имя [Отчество] Фамилия». Предлоги остаются от
// срезанных цифр («за 2500 галина…» → [за, галина, …]) — имя с них не
// начинается, а в name-matcher они только мешали бы.
const FILLER = new Set([
  "так", "давай", "давайте", "значит", "ну", "вот", "это", "так-то",
  "пожалуйста", "итак", "и", "а",
  "за", "на", "по", "от", "для", "у", "в", "с",
]);

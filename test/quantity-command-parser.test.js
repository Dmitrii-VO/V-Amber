...
test("past-tense / description forms do NOT trigger (opencode review)", () => {
  // «Анна добавила две штуки 03204» — пересказ оператором того, что
  // сделал зритель. Команда — только повелительное наклонение.
  assert.equal(
    parseQuantityCommand("Анна Сидорова добавила две штуки 03204").matched,
    false,
  );
  assert.equal(
    parseQuantityCommand("Анна Сидорова поменяла бы две штуки 03204").matched,
    false,
  );
  assert.equal(
    parseQuantityCommand("Анна Сидорова изменилась две штуки 03204").matched,
    false,
  );
});
...
test("составное «двадцать пять штук» → 10 / 25 (не тихий no-op)", () => {
  const r = parseQuantityCommand("Анна Сидорова добавь двадцать пять штук 03204");
  assert.equal(r.matched, true);
  assert.equal(r.quantity, 10);
...
test("составное «тридцать две штуки» → 10 / 32", () => {
  const r = parseQuantityCommand("Иван Петров добавь тридцать две штуки 12345");
  assert.equal(r.matched, true);
  assert.equal(r.quantity, 10);
  assert.equal(r.requested, 32);
});

test("составное с «пара» удваивает («двадцать одна пара» → 21*2=42 → 10)", () => {
  const r = parseQuantityCommand("Анна Иванова добавь двадцать одну пару 03204");
  assert.equal(r.matched, true);
  assert.equal(r.quantity, 10);
  assert.equal(r.requested, 42);
});

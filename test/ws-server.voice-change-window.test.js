...
test("повторный артикул с первой ценой переоценивает существующую бронь", async () => {
  const harness = await startHarness({ cardsByCode: { "03048": CARD }, knownCodes: ["03048"] });
  const client = await harness.connect();
  try {
    ...
    const updates = harness.moysklad.callsTo("updateCustomerOrderPositionPricing");
    assert.equal(updates.length, 1);
    assert.equal(updates[0].args[0].salePrice, 8800);

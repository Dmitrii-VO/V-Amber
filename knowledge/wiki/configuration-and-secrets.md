...
VK (с 13.09.2026 — только токен сообщества, пользовательский убран, см.
[[vk-integration]]):
...
- `VK_GROUP_TOKEN` / `VK_ACCESS_TOKEN` — токен сообщества Amberry,
  ...
  **Должен принадлежать той группе, от чьего имени должны идти ответы
  ...
  переменную не коммитить. Это **единственный** VK-токен: под ним идут приём
  комментариев (Long Poll), публикация от имени сообщества, ЛС и бан. Для проверки в дашборде смотрите footer/диагностику
  — там виден `groupId`, а ответы должны приходить от Amberry.
- `VK_LIVE_VIDEO_URL`
- `VK_LIVE_POST_ID` — запись на стене, к которой привязано видео эфира;
  под неё публикуются карточки лота и подтверждения броней
  (`wall.createComment` от имени сообщества). Обычно узнаётся сама из первого
  комментария зрителя, задавать вручную нужно только чтобы публиковать до
  него. См. [[vk-integration]].
- `VK_GROUP_ID`

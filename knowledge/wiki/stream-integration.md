...
## Карточка лота на странице зрителя (2026-08-03)

Замечание Романа: «на нашей платформе не высвечивается лот, когда его
называют». Так и было — не регрессия, а дыра: `vk.publishLotCard` публиковал
лот **только** в VK-комментарии, а в сторону `/efir/` из V-Amber уходили
всего два типа сообщений (ответ на бронь и периодическая инструкция). Зритель
своей площадки видел видео и чат, но ни названия, ни цены, ни фото лота.

Теперь лот едет и туда:

- **`server/viewer-lot.js`** — `buildViewerLot(lot)` (снимок: код, название,
  категория, цена с учётом скидки, базовая цена, остаток, статус) и
  `createViewerLotPublisher()`. Публикатор дедупит по сигнатуре снимка:
  точка вызова — `emitState()` в `server/ws-server.js`, а он срабатывает
  десятки раз в минуту (транскрипты, брони). Одна точка входа покрывает все
  изменения лота — открытие, цену голосом, ручную цену, скидку, закрытие —
  поэтому вызовов рядом с каждым `vk.publish*` нет.
- **Фото** шлётся байтами (`photo.base64`) и только при смене артикула:
  картинка МойСклада живёт за авторизацией, публичной ссылки нет. Смена лота
  без фото шлёт явный `photo: null`, иначе на новой карточке осталась бы
  картинка предыдущей.
- **Закрытие лота** не убирает карточку, а переводит её в `status:"closed"`
  («Лот закрыт · 03204») — аналог отдельного комментария `publishLotClosed` в
  VK. Совсем снимается она на остановке эфира/закрытии сокета
  (`resetDetectionState()` → `viewerLot.clear()`).
- **Канал best-effort**: ошибка чата — только WARN `chat/lot_card_publish_failed`,
  плюс сброс дедупа, чтобы следующее изменение попробовало снова. Эфир,
  VK и МойСклад от этого канала не зависят.
- **`deploy/chat-service`** держит одну текущую карточку (`POST /chat/lot`
  под `X-Chat-Token`, публичные `GET /chat/lot` и `GET /chat/lot/photo`),
  переживает рестарт контейнера через `data/lot.jsonl` + `data/lot-photo.bin`
  и отдаёт карточку прямо в ответе `GET /chat/messages` — страница и так
  опрашивает его раз в 3 секунды, второй поллер был бы лишним.
- **`deploy/stream-viewer`** — карточка в правой колонке над чатом
  (`#lotCard`), рядом с ней подсказка «Бронь: напишите в чат "бронь 03204"».
- Тесты: `test/viewer-lot.test.js` (снимок, дедуп, политика фото, sticky
  closed, ошибка чата, keepalive) и `test/ws-server.viewer-lot.test.js`
  (сквозной путь открытие → закрытие → остановка эфира).

**Деплой**: половина в `deploy/**` (уезжает автодеплоем по пушу в `main`),
половина в `server/**` (только через релиз и ручное обновление на Mac
оператора) — как и в разделе про сброс сессии чата выше, до обновления обеих
половин карточка работать не будет.

### Карточка не переживает молчание V-Amber (2026-08-04)

У карточки не было срока годности: V-Amber умер посреди эфира или оператор
закрыл ноутбук, не остановив эфир, — и `/efir/` показывал лот со старой ценой
до тех пор, пока следующий эфир не пришлёт новую карточку. `viewerLot.clear()`
тут не помогает: он вызывается только когда V-Amber жив и корректно
останавливает эфир — то есть ровно не в этом сценарии.

Лечится тем же приёмом, что и плашка «смотреть в ВК», но с одной поправкой:
**карточка персистится и законно висит без изменений минутами**, поэтому
«гасить через N минут после публикации» нельзя — TTL считает возраст не
карточки, а последнего подтверждения от V-Amber.

- **Heartbeat в `server/viewer-lot.js`** (`LOT_HEARTBEAT_MS`, 5 минут):
  публикатор сам держит таймер, пока карточка висит (в том числе закрытая),
  и шлёт `POST /chat/lot {keepalive:true}` (`chatClient.confirmLot()`).
  Таймер перезаводится от каждой успешной публикации — она и есть
  подтверждение — и снимается в `clear()`. Дедуп по сигнатуре тут принципиально
  не годится: он про «ничего не изменилось», а подтверждать надо именно это
  состояние.
- **TTL в `deploy/chat-service/server.js`** (`LOT_TTL_MS`, 20 минут = четыре
  такта): `currentLotCard()` отдаёт `currentLot`, только пока подтверждение
  свежее, и подставлен во все три выдачи — `GET /chat/lot`, `GET /chat/messages`
  и `/chat/health`. Запас в четыре такта — чтобы пара потерянных keepalive'ов
  из-за сети не гасила карточку в идущем эфире.
- **Keepalive не трогает `rev` и не пишет в журнал.** `rev` сидит в URL фото
  (`/chat/lot/photo?rev=N`), и его рост на каждом такте перекачивал бы зрителям
  картинку раз в 5 минут; журнал бы пух на ровном месте.
- **Протухание read-only**: ни `currentLot`, ни `lotPhoto` не стираются, просто
  не отдаются. Поэтому вернувшийся V-Amber поднимает ту же карточку одним
  keepalive'ом — а если бы фото стёрли, публикатор его не переслал бы (фото
  уходит только при смене артикула) и карточка осталась бы без картинки.
- **Рестарт контейнера** больше не воскрешает карточку прошлого эфира: возраст
  подтверждения восстанавливается из `ts` последней строки `lot.jsonl`, а не
  обнуляется. Цена — до 5 минут без карточки после рестарта посреди эфира.
- **Почему не подвесили это на `POST /chat/state`** (heartbeat из
  `server/cross-promo.js`, тоже раз в 5 минут): он уходит, только пока
  ВК-зеркало реально в эфире (`syncMirrorBanner` шлёт пустую строку лишь один
  раз), и его целиком выключает `CROSS_PROMO_ENABLED=0`. Карточка на своей
  площадке гасла бы из-за состояния ВК — связь, которой быть не должно.
  Подтверждает карточку тот, кто ей владеет.

## Перекрёстные подсказки между площадками (2026-08-04)

Запрос Романа: бот должен писать зрителям, что при плохом качестве картинки
или звука можно смотреть по ссылке — **в обе стороны**, потому что эфир идёт
одновременно в ВК и на своей площадке.

### Как на самом деле устроен мультистрим (важно)

Дубль в ВК делает **не наш ffmpeg-релей**, а плагин OBS
[obs-multi-rtmp](https://github.com/sorayuki/obs-multi-rtmp/releases/) — так
сказал Роман, и логи это подтверждают: в `server.log` за всё время **ноль**
строк с `relay`, а `broadcast_started` встречается только 2026-07-20 (тесты).
В эфире 2026-08-01 его нет — поток запускают руками из OBS. Отсюда два
следствия:

- **`server/stream-relay.js` в проде не используется.** Гейтить подсказки по
  его состоянию нельзя: оно всегда `idle`, хотя ВК-эфир идёт.
- «Остановить» в дашборде (obs-websocket `StopStream`) гасит только основной
  выход OBS; цели плагина продолжают пушить. И `applyObsPreset` принудительно
  ставит `Output/Mode=Simple` — мультистрим-сетапы обычно живут в Advanced,
  так что на время работы с плагином оператору лучше `OBS_APPLY_PRESET=0`.

### Механика

`server/cross-promo.js` — один таймер, два такта:

- каждые `CROSS_PROMO_PROBE_INTERVAL_MIN` (5) обе площадки проверяются
  **по-настоящему**: своя — `getStreamStatus()` (MediaMTX `ready`, не зависит
  от того, чем пушат поток), ВК — `vk.validateLiveVideoUrl()`
  (`live_status === "started"`, метод уже существовал). Ошибка пробы = «не в
  эфире»: мёртвая ссылка хуже молчания;
- раз в `CROSS_PROMO_INTERVAL_MIN` (25) уходят сами сообщения, и только когда
  живы **обе** площадки: в ВК-комментарии — со ссылкой на свою площадку
  (`STREAM_VIEWER_URL`), в чат `/efir/` — про ВК. Тексты разные для каждого
  канала: существующие `publishViewerInstruction` шлют один текст в оба
  канала сразу, и подсказка «переходите на другую площадку» так работать не
  может. Варианты чередуются — ВК режет одинаковые подряд идущие комментарии.

Ссылки нигде не дублируются в конфиге: адрес ВК берётся у того же видео, у
которого читаются комментарии (`vk.getLiveVideoUrl()`), поэтому не может
протухнуть при новом эфире.

### Плашка на странице зрителя

В чате `/efir/` ссылка текстом бесполезна: сообщения рисуются `textContent`
(защита от разметки), URL был бы некликабельным. Поэтому кликабельная ссылка
живёт в плашке под плеером, а сообщение бота просто показывает на неё.

- V-Amber пушит `POST /chat/state {vkMirrorUrl}`; chat-service держит это
  состояние **в памяти и по TTL** (`BROADCAST_STATE_TTL_MS`, 12 минут):
  V-Amber подтверждает его каждые ~5 минут, и если он замолчал (упал, эфир
  кончился), плашка гаснет сама. Логи 2026-08-01 показывают, что канал до
  чата иногда отваливается по таймауту (`chat_poll_failed` ×4 → recovered),
  так что «само чинится» здесь обязательно.
- Ссылка уходит в `href`, поэтому chat-service принимает **только** `https`
  и только домен `vk.com` — не «любую строку от оператора».
- Плашка видна всем, пока ВК-эфир идёт, но в спокойном виде. Если плеер
  реально спотыкается (`bufferStalledError` в hls.js, `waiting` в Safari —
  два события за минуту), она подсвечивается и меняет текст на «Видео
  подтормаживает…», а через 3 минуты без заминок возвращается в спокойный
  вид. Так подсказка попадает именно тем, у кого проблема, и не мозолит
  глаза остальным.

### Грабля: `server/config.js` тянет `dotenv/config`

Первая версия импортировала `getStreamStatus` прямо в `server/ws-server.js` —
и весь тестовый прогон встал: `stream-status.js` → `config.js` →
`import "dotenv/config"`, то есть `.env` разработчика влился в `process.env`
тестов (включив им `API_TOKEN` — все WS-тесты получили 401 — и подставив
настоящие адреса MediaMTX/ВК). **`ws-server.js` не должен импортировать
`config.js` ни транзитивно**: пробу инжектит `server/index.js` через
`services.getStreamStatus`. Родственная грабля описана в [[testing-guide]].

Тесты: `test/cross-promo.test.js` — матрица live/не-live для обеих площадок,
падение проб, ротация вариантов, «сообщения не чаще интервала, плашка каждым
тактом», снятие плашки на `stop()`.

## «Сейчас смотрят N» на странице зрителя (2026-08-04)

Строка `👁 N зрителей` рядом с заголовком `/efir/`. Считает **открытые
страницы**, а не читателей HLS: настоящие readers знает только MediaMTX, чей
control-API сидит на loopback за токеном и странице недоступен, — а зрителя,
который ещё не представился в чате (плеер под auth-гейтом) или ушёл смотреть в
ВК по плашке-зеркалу, не закрыв вкладку, в зале всё равно стоит показывать.

Механика — в самом опросе чата, без второго поллера и без heartbeat-эндпоинта:
`GET /chat/messages` (раз в 3с) отмечает клиента в памяти chat-service и
возвращает `online` тем же ответом, что ленту, карточку лота и плашку ВК.

- Ключ — случайный `clientId` из `localStorage` в заголовке `X-Efir-Client`:
  перезагрузка и вторая вкладка одного зрителя = один человек. Нет заголовка
  (старая закэшированная страница) — ключом становится хэш IP+User-Agent; в
  память кладётся только хэш.
- `ONLINE_TTL_MS` = 15с (~5 тактов опроса): счётчик не мигает из-за одного
  потерянного запроса, закрытая вкладка уходит из счётчика за 15с. Ничего не
  персистится.
- Опрос V-Amber (`X-Chat-Token`) исключён — иначе оператор считался бы зрителем
  весь эфир.
- Ноль не показываем: зритель на странице — уже единица, `online: 0` означает
  лишь, что сервис ещё не успел его засчитать (или ответ старого формата).
- Число в панели «Стрим» дашборда (`readers` от MediaMTX) и это число —
  РАЗНЫЕ величины и расходятся штатно. Подробности в
  `deploy/chat-service/README.md § Счётчик «сейчас смотрят»`.

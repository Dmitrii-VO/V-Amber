...
## Post-broadcast review («🔎 Разбор», 2026-08-24)

Строки «требует внимания» переживают эфир и разбираются после него.

**Зачем.** Строка возникает, когда покупатель написал код, а однозначного
открытого лота под него нет: лот закрыли раньше, он был в другой день кампании,
или код подошёл нескольким лотам сразу. До 2026-08-24 она жила только в баннере
во время эфира — и по логам 13 эфиров этим не воспользовались **ни разу**: строк
было 6488, созданных из них броней 0. Причина не в баннере: во время эфира
оператор держит телефон как камеру и в ноутбук не смотрит, поэтому любой
механизм «система заметила → человек посмотрит и нажмёт» превращается в «ничего
не произошло». Разбор перенесён туда, где глаза и руки свободны.

- **Хранилище** — `server/attention-store.js`, append-only JSONL
  (`logs/attention.jsonl`, как blocked-viewers). Строка живёт 14 дней
  (`retentionDays`), потом уходит из списка сама.
- **Кнопка «🔎 Разбор»** в шапке со счётчиком открытых строк.
- **«Забронировать»** уходит по WS сообщением `reserveFromAttention` с `rowId` —
  тем же денежным путём, что и живой баннер (`actionId`). Второй точки записи в
  МойСклад не заводится сознательно: у этого пути есть проверка дубля, поиск
  заказа кампании и маршрутизация через открытый лот, если он успел открыться.
- **«Снять»** — `POST /api/attention/dismiss`, закрывает строку без брони.
- **Отказ строку НЕ закрывает** (safe-mode, МойСклад недоступен): оператор
  повторит. Закрывается только успех.
- **`ambiguous` в разбор попадает, но кнопки брони не получает** — код подошёл
  нескольким лотам, выбирать товар за оператора в денежном пути нельзя.

...
- **Автоперенос из брака, 2026-08-29**: если по разрешённым складам остаток
  ...
  не меняет, а весь смысл именно в остатке. Приёмник выбирается из РАЗРЕШЁННЫХ
  складов, а не из `defaults.storeId`: тот при пустом `MOYSKLAD_STORE_ID`
  берётся из `stores[0]`, а в боевом аккаунте первым идёт как раз брак.

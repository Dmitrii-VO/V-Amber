...
- **Источник остатка сменился, 2026-08-29**: `report/stock/all` по части товаров
  ...
  товарам отвечает корректно. Поэтому `report/stock/all` (с фильтром по
  складам) остался основным источником, а `bystore` зовётся только когда
  агрегат промолчал или продавать нечего: лишний round-trip стоит ~280 мс, а
  карточка грузится 143 раза за эфир. Отличать «товара нет нигде» (ноль) от
  «запрос не удался» (`null`) обязательно: первое не должно снимать бронь у
  оператора, который держит товар в руках.
- **Склады исключаются по префиксу имени, не по равенству**: боевой склад
  ...
  остатком (23 SKU / 26 шт, 17 из них не лежат больше нигде). Остаток по исключённым
...
- **Автоперенос из брака, 2026-08-29**: если по разрешённым складам остаток
  ...
  Документ создаётся с `applicable: true` — непроведённое перемещение остаток
  не меняет, а весь смысл именно в остатке. Приёмник — `defaults.storeId`
  (он учитывает `MOYSKLAD_STORE_ID` и `preferredStoreName` «Основной склад»),
  но только если сам не исключён; иначе первый разрешённый склад. Проверять
  обязательно: при пустом `MOYSKLAD_STORE_ID` и несовпавшем имени
  `defaults.storeId` берётся из `stores[0]`, а в боевом аккаунте первым идёт
  как раз брак — а первым РАЗРЕШЁННЫМ идёт «Аукцион», тоже не склад продаж.
  Перенос закрыт `wrapWithSafeMode` и делается один раз на товар за эфир —
  через write-журнал он не проходит и не идемпотентен.
...
- [[../raw/log-review-2026-08-29|log-review-2026-08-29]] — разбор, из которого
  вышли и смена источника остатка, и исключение склада брака

...
Сначала — **групповой токен**, на котором с 13.09.2026 держится весь эфир:
сообщество отвечает, Long Poll включён, событие «Комментарий к видео:
добавлен» стоит, очередь выдаётся, имена зрителей читаются, ЛС доступны, и
известна ли запись для публикации (`VK_LIVE_POST_ID`; пустая — не беда,
узнается из первого комментария). Красный пункт здесь означает, что эфир
будет глухим.

Потом — **пользовательский токен**, оставшийся запасным путём. Его `ошибка 9`
(флуд-блок) эфиру больше не мешает, но без него не прочитать комментарии
прошедшего эфира, то есть не восстановить заказы по логам.

См. [[runbooks-and-troubleshooting]] и [[vk-integration]].

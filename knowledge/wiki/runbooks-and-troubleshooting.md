...
## Комментарии ВК не приходят (флуд-блок аккаунта)

Симптом: брони не регистрируются, бот не отвечает, конкурс глухой
(`attempts: 0`), в ленте дашборда пусто. На дашборде висит баннер
«Комментарии ВК не приходят».

1. `node scripts/vk-token-check.js` — только чтение, ничего не публикует.
2. `ОШИБКА 9: Flood control` у `video.getComments` → ВК заблокировал аккаунт
   `VK_USER_TOKEN` по флуду. Это блок **аккаунта**, не токена: перевыпуск
   токена тем же человеком не помогает.
3. Быстрое лечение — `VK_USER_TOKEN` другого администратора сообщества
   (scope `video,groups,offline`), подставить в `.env` и перезапустить.
   Служебные комментарии будут подписаны его именем.
4. Иногда блок снимается после входа в ВК браузером под этим аккаунтом и
   прохождения капчи.
5. Вход комментариев при этом можно не терять: Long Poll сообщества работает
   под групповым токеном — см. [[vk-integration]]. Публикация в ВК всё равно
   ждёт живого user-токена.

Разбор эфира, где это случилось: 2026-09-12, [[vk-integration]].

## Порт 8080 занят

С 13.09.2026 приложение сначала выясняет, **кто** занял порт:

- своя вторая копия (страница отвечает разметкой V-Amber) → как и раньше,
  вторая копия завершается с сообщением «V-Amber уже запущен»: два эфира
  рядом — это двойные публикации в ВК;
- чужое приложение (в разборе это был веб-интерфейс qBittorrent) → уступаем
  порт и поднимаемся на следующем свободном: 8080 → 8081 → …  Реальный адрес
  печатается в консоль и пишется в `logs/server-url.txt`, откуда его берёт
  `start.command` и открывает нужную вкладку.

Если свободного порта рядом не нашлось — выход с подсказкой задать порт
вручную (`PORT=8099` в `.env`).

...
**Проверить через API не вышло.** С этой Windows-машины `api.moysklad.ru`,
`online.moysklad.ru` и `dev.moysklad.ru` недоступны — TCP-connect отваливается
по таймауту, при этом github.com отвечает 200. Ровно поэтому вопрос про
`syncId` уже висел непроверенным с прошлой итерации. Записал в
[[project-conventions#Dev machine limits]], чтобы третий раз не наступать.
Для проверки там, где сеть есть, положил `scripts/probe-moysklad-syncid.mjs`:
без флагов только читает, с `--write-test` делает два POST с одним syncId и
говорит, обновился заказ или создался второй. Код на upsert НЕ рассчитывает —
защита держится журналом и поиском.

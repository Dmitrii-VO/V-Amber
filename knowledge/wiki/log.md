...
`main`). 550 тестов зелёные на каждом шаге.

**Срезано.** Мёртвые экспорты `LOT_HEARTBEAT_MS`/`viewerLotSignature`;
`.obsidian/` из индекса (206 строк личного состояния редактора в публичном
репозитории); `scripts/roman-bridge.js` и `roman-watch.js` (VK-мост к одному
человеку, ни на что не завязан); `parseCsvEnv`+`parsePipeEnv` → один
`parseListEnv`; 11 env-ручек → константы (см.
[[configuration-and-secrets#What is deliberately not configurable (2026-08-06)]]);
дубли нормализации русского текста → `ru-numerals.js`; общий движок JSONL →
`server/jsonl-store.js`; два модуля из `ws-server.js` (`viewer-instructions`,
`pending-actions`), 4415 → 4314 строк.

**Аудит завысил втрое, и это главный вывод.** Обещал −2400 строк, по факту
≈ −480. Три ошибки, все в одну сторону:

- «~30 мёртвых env-ручек» → 11. Смотрел только `.env.example` и не проверил
  wiki, где `OBS_*`, `CROSS_PROMO_*` и часть `MOYSKLAD_*` документированы.
- «13 копий `normalize()`» → 5. Сгруппировал по имени, а `normalizeMoney`,
  `normalizeKnownCodes`, `normalizeVkOwnerId` делают разные вещи.
- «объединение store-модулей: −760» → −15. Принял суммарный объём шести
  файлов (1265 строк) за объём дублирования. Почти всё там — доменная логика,
  у каждого стора своя.

Мораль для следующего аудита: считать пересечение, а не сумму, и проверять
wiki прежде чем называть настройку недокументированной.

**Что решили НЕ делать.** Замену самописного `zip-writer.js` (99 строк) на
`ditto`: проверить с Windows-машины нельзя, а правка ушла бы в релиз вслепую,
и она меняет чистый Node на внешний процесс. Общий `writeJsonAtomic` для
`state-store`/`settings-store`/`product-code-cache`: совпадает 4 строки, а у
`state-store` запись сплетена с защёлкой `discardSaves`. Разбивку
`web-ui/app.js` (4116 строк): тестов на web-ui нет вообще, а МойСклад с
машины разработки не отвечает — проверять нечем.

**Грабля с мержем стека PR.** `gh pr merge --delete-branch` на нижнем PR
стека удаляет базовую ветку следующего, и GitHub автоматически ЗАКРЫВАЕТ его.
Переоткрыть или сменить базу у закрытого PR нельзя, пока базовой ветки нет:
пришлось запушить удалённую ветку обратно, переоткрыть PR, перенацелить на
`main`. Правильный порядок: мержить весь стек без `--delete-branch`,
перенацеливая каждый следующий на `main`, и чистить ветки в самом конце.
Смена базы перезапускает CI — мержить после зелёного прогона.

## [2026-08-05] audit | Промежуточные распознавания: гипотеза не подтвердилась, но замер нашёл три других дефекта

Вопрос был: открывать ли лот по промежуточному распознаванию SpeechKit, если
артикул дважды подряд появился в партиале. Ответ по данным — нет, и ни одна из
предпосылок гипотезы не подтвердилась.

**Данные.** Шесть эфиров в `logs/` (`_audit`, `_diag_0605`, `_ext1`, `_ext2`,
`extracted_0627`, `sessions`), после дедупа по имени+размеру — 43 уникальных
session-jsonl, 19 393 события, 6436 финалов, 525 открытий лота, 24.05–27.06.
Осторожно с нормализацией слов-цифр: замена «семь» съедает «восемь», длинные
слова заменять первыми — на этом первый прогон дал 75 ложных «код не найден»
вместо 8.

**Почему правило «дважды подряд» не работает.** Разрывов фразы нет: код целиком
виден в ОДНОМ финале в 449 из 457 голосовых открытий, склейка окна триггеров
(8 с, буфер 3) не понадобилась ни разу. Выигрыш по времени мал: медиана слов
после кода в той же фразе — 0, только 13% фраз с кодом содержат ещё и цену, то
есть финал приходит почти сразу и партиал экономит паузу EOU (700 мс), а не
секунды. И ложные срабатывания устойчивы к повтору: фрагмент цены повторится в
партиалах так же надёжно, как верный код.

**Что замер нашёл вместо этого (все три починены).**

1. Задержка «финал с кодом → `lot_opened`»: p50 1836 мс, p90 7680 мс, быстрее
   полусекунды только 7%. Вызовы МойСклада в этом окне — 150–500 мс, остальное
   съедал `await vk.publishLotCard` на критическом пути. Лот теперь открывается
   не дожидаясь карточки; `commentId` прикрепляется вторым состоянием. Брони не
   разъезжаются: поллер фильтрует по своему курсору, комментарий привязывается
   по коду, а лотовый `lastCommentId` вообще нигде не читается.
2. Сверка 525 открытий с настоящим каталогом (2407 товаров, выгружен через
   туннель): 520 сошлись, 5 — нет, и все пять семизначные, где к артикулу
   приклеился размер («артикул 03413 пятьдесят сантиметров» → `0341350`). Все
   пять из ОДНОЙ сессии 27.06, где 78 из 78 вызовов МойСклада упали: каталожный
   гейт включается только на непустом кеше, а кеш был чисто в памяти. Кеш кодов
   теперь переживает перезапуск и падение МС на диске, пустой ответ каталог не
   затирает. Границы длины артикула тоже берутся из каталога (в нём коды 2, 3,
   5 и 6 знаков) вместо `.env` 1..10; нижняя считается ПОСЛЕ снятия ведущих
   нулей, иначе сломается штатное «пятьсот восемьдесят восемь» → 00588.
3. Партиалы не писались никуда. Гипотезу из первого абзаца нельзя было
   проверить задним числом ни по одному бандлу. Теперь пишутся как
   `transcript_partial` с `seq` внутри реплики; повтор неизменившегося текста
   отбрасывается.

Порядок расследования, который стоит повторять: сверять открытия лотов не с
догадками, а с выгруженным каталогом, и всегда смотреть, не сгруппированы ли
аномалии в одной сессии — 49 из 55 «открытий без товара» оказались артефактом
той же упавшей сессии, а не системной проблемой точности.

`npm test`: 531/531. PR #45, #46, #47.

## [2026-08-05] fix | syncId у закупочного заказа + честный статус «не знаю»

Продолжение записи ниже. Там защита от дубля держалась на отпечатке группы —
поставщик, склад, точное описание, состав позиций. Это работает, но это
угадывание: описание собирает шаблон оператора, и два одинаковых заказа
делают сверку беспомощной.

Теперь `createPurchaseOrder` штампует заказ полем `syncId` —
детерминированный UUIDv5 из `draftId::groupHash`
(`buildPurchaseOrderSyncId`). Это внешний код на стороне МойСклада, оператор
его не видит, поэтому — в отличие от брони — метку не пришлось прятать в
описание. Сверка стала точной: `filter=syncId=<uuid>`, одна строка — заказ
есть, две — «не знаю».

**Пустой ответ по syncId ничего не доказывает** (заказ мог уйти со старой
версии приложения, без syncId; сам фильтр может быть недоступен), поэтому
вердикт «не применилось» произносит по-прежнему только слой отпечатка. Два
слоя, точный первым, эвристика запасной.

**Проверено на рабочем аккаунте, и ответ — да.** С этой Windows-машины
`api.moysklad.ru`, `online.moysklad.ru` и `dev.moysklad.ru` недоступны:
TCP-connect отваливается по таймауту, при этом github.com отвечает 200. Ровно
поэтому вопрос про `syncId` висел непроверенным с прошлой итерации. Обход —
SSH-туннель через машину, у которой доступ есть, и `curl --connect-to`: ключи
МойСклада при этом не покидают локальную машину, по туннелю идёт только TLS.
Записано в [[project-conventions#Dev machine limits]].

Результат прогона: `filter=syncId=<uuid>` поддерживается (вернул `size: 0` при
существующем заказе — значит фильтр реально применился), а **повторный POST с
тем же syncId вернул id и name первого заказа**, и по фильтру нашёлся ровно
один. `syncId` — настоящий ключ идемпотентности, дубль на стороне МойСклада
невозможен. Слой отпечатка из-за этого стал подстраховкой, а не несущей
конструкцией; убирать его пока не стал.

Две грабли API, на которые потратилось время: без `Accept-Encoding: gzip`
nginx отдаёт 415 (undici шлёт его сам, curl — только с `--compressed`), а
переформатированное тело запроса даёт ошибку 2001. Обе записаны в
[[runtime-stores]]. Скрипт `scripts/probe-moysklad-syncid.mjs` оставлен, чтобы
перепроверить, если МойСклад поменяет поведение.

**Оператору.** Неизвестный исход перестал притворяться ошибкой: группа
пишется со статусом `unknown`, уезжает в ответе отдельным `unknownGroups`, и
интерфейс говорит прямо — заказ мог создаться, проверьте закупки перед тем,
как делать руками; повторная отправка из V-Amber дубль не создаст.

`npm test`: 518/518.

## [2026-08-05] fix | Закупочный заказ: защита от дубля при потерянном ответе

Отправка списка желаний уже была идемпотентна через `wishlist-submissions`, но
результат группы сохраняется ПОСЛЕ ответа МойСклада. Если POST
`entity/purchaseorder` доехал, а ответ потерялся (таймаут, 5xx, обрыв), группа
записывалась как ошибка — и повторная отправка создавала второй закупочный
заказ.

`createPurchaseOrder` заведён в журнал внешних записей с ключом
`po::${draftId}::${groupHash}`: обе части уже считаются на горячем пути,
`groupHash` — детерминированный отпечаток группы (поставщик + склад + состав
позиций), поэтому соседние группы одной отправки законно получают разные
ключи. Сверка неизвестного исхода не может опираться на маркер в описании —
описание собирается по шаблону оператора и читается людьми, — поэтому ищет
заказ по отпечатку: 50 свежих заказов поставщика, фильтр по складу и точному
описанию, затем сравнение состава позиций. Одно совпадение — «применилось»,
два и больше (или больше пяти кандидатов по описанию) — «не знаю», без
повтора и без выдумывания успеха.

Остальные записи в МойСклад намеренно не трогали: удаление позиции уже
идемпотентно (повторный DELETE с 404 = успех), изменение цены задаёт
абсолютное значение, создание контрагента сперва ищет существующего по VK ID,
имени и описанию. Подробности — [[runtime-stores#MoySklad write journal]].

## [2026-08-04] feat | «Сейчас смотрят N» на странице зрителя

Зритель на `/efir/` не видел, есть ли рядом кто-то ещё. Настоящих читателей
потока знает MediaMTX, но его control-API живёт на loopback за токеном и
странице недоступен — да и считать надо не только их: зритель под auth-гейтом
(ещё не представился) и ушедший смотреть в ВК по плашке-зеркалу в зале всё
равно присутствуют. Поэтому считаем открытые страницы, и делаем это внутри
уже существующего опроса чата: `GET /chat/messages` раз в 3с отмечает клиента
и возвращает `online` тем же ответом — ни второго поллера, ни heartbeat-роута.

Ключ присутствия — случайный `clientId` из `localStorage` в заголовке
`X-Efir-Client` (перезагрузка и вторая вкладка = один человек), для старых
закэшированных страниц — хэш IP+User-Agent. TTL 15с. Опрос V-Amber
(`X-Chat-Token`) исключён, иначе оператор весь эфир считался бы зрителем.

Проверено на локальном стенде: 1/2/5 зрителей рисуются с правильными
окончаниями, перезагрузка страницы не удваивает счётчик, операторский опрос
не увеличивает, через 15с без опроса «ушедшие» исчезают. Подробности —
[[stream-integration#«Сейчас смотрят N» на странице зрителя (2026-08-04)]].
## [2026-08-04] feat | Зал и чат зрителей — одна лента, превью сворачивается

Оператор сообщил: чата клиентов не видно вообще. Причина не в данных, а в
раскладке — центральная колонка держала три панели (`auto 1fr 1fr`), превью
съедало ~245px, и на 13" ноутбуке «Чат зрителей» схлопывался до шапки и поля
ввода: сами сообщения занимали 0px. Панель `#chatPanel` убрана, её содержимое
переехало в `#commentsPanel`: одна лента (VK + чат + сервисные ответы «Янтарь»
+ разделители сессии) и поле ответа внизу той же панели.

Сообщения чата рисует только HTTP-опрос — он работает без голосовой сессии,
тогда как WS-события идут лишь во время эфира. Из WS для чата берётся один
`viewerId` (в публичной ленте его нет) — им включается «🚫» на уже отрисованной
строке; сшивка по `seq`, потому что chat-service выдаёт `commentId = 9e9 + seq`.
Порядок прихода произвольный, поэтому связь двусторонняя.

Плюс `#previewToggle` — превью сворачивается (плеер продолжает играть, звук
эфира не теряется), состояние живёт в `localStorage["previewCollapsed"]`,
потолок панели снижен с 42vh до 34vh. Замер на 1280×720 в «Свой эфир»: лента
176px, со свёрнутым превью 336px (было 0). Подробности —
[[web-dashboard#One hall feed instead of two panels (2026-08-04)]].

...
## Отмена без артикула (2026-08-29)

«отмена» / «отменить» без кода больше не всегда уходит оператору. Если во всём
эфире у покупателя ровно **одна** живая бронь на открытых лотах **и в его
заказе ровно одна позиция** — она снимается сразу (`path: "no_code_single"`).
Вторая проверка обязательна: бронь могла остаться и на ЗАКРЫТОМ лоте, в памяти
её нет, а в заказе есть, и «единственная» бронь оказалась бы не единственной.
Заказ берётся из сессионного кеша, в МойСклад лезем только если там пусто; не
удалось выяснить число позиций — автоотмену не делаем. Ноль или несколько — прежнее поведение:
предупреждение оператору и ответ покупателю с форматом «отмена 03204». Угадывать
при нескольких бронях нельзя: снимется не тот товар, и покупатель этого не
заметит.

За эфир 29.08 таких комментариев было шесть (Балашова написала так трижды,
потом сама догадалась добавить артикул). Ответ покупателю работал и раньше —
в логах шесть `status: cancel_no_code`.

**Опустевший заказ удаляется.** Когда снята последняя позиция,
`removePositionFromOrder` удаляет и сам заказ, возвращая `orderDeleted: true`;
вызывающий сбрасывает `customerOrdersByViewerId`, иначе следующая бронь
покупателя дописалась бы в несуществующий заказ. Проверка «пуст ли заказ»
падает — заказ не трогаем. При 404 на удаление позиции (сняли не мы) заказ не
трогаем тоже: пустым он мог стать в чужих руках. 29.08 такими остались VK03154
и VK03163.

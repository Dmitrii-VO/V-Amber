...
| `/api/stream/status` | `GET` | Returns `{configured, live, readers, error?, relay}` by polling the MediaMTX control API. Degrades to `live:false` on any failure — never throws. `relay` = `{configured, state (idle\|running\|error), restarts, lastError}` for the VK dual-stream ffmpeg relay (see [[stream-integration#Dual-stream: mirror the эфир to VK (2026-07-22)]]). |

...
- Извлечение артикула из финальных транскриптов по regex/number-word parsing и
  кешу кодов товаров из МойСклад.
- Детекция голосовой скидки и публикация обновленной цены в VK.
...
- `VK_API_MIN_INTERVAL_MS`, `VK_API_RATE_LIMIT_BACKOFF_MS` - паузы между
  запросами к VK API и backoff при `VK API 6`.

VK и МойСклад на уровне кода опциональны: без настройки часть действий будет
пропущена или залогирована. SpeechKit API key обязателен.
...
В верхней панели dashboard есть кнопка «Скачать логи». При клике открывается
окно с полем «Что случилось?», списком файлов и кнопкой «Скачать ZIP».
...
Скачивание архива не ограничено rate-limit.

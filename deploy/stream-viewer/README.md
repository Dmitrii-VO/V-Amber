...
Публичная страница просмотра эфира: `index.html` + вендоренный `hls.min.js`
(hls.js 1.6, скачан с jsdelivr). Живёт на хосте `cloud` рядом с MediaMTX,
раздаётся тем же nginx, что и `www.xn--80azkg6cn.space`.
...
scp deploy/stream-viewer/index.html deploy/stream-viewer/hls.min.js cloud:/tmp/
ssh cloud "sudo mv /tmp/index.html /tmp/hls.min.js /var/www/stream-viewer/ \
